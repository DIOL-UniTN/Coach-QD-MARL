...
from decisiontreelibrary import (ConditionFactory, QLearningLeafFactory,
                                 RLDecisionTree)
from magent2.environments import (adversarial_pursuit_v4, battle_v4,
                                  battlefield_v5, combined_arms_v6,
                                  tiger_deer_v4)
...
def evaluate(tree, config):
    ...
    from magent2.environments import (adversarial_pursuit_v4, battle_v4,
                                      ...
                                      tiger_deer_v4)
    agents = {}
    env = eval(config["env"]["env_name"])
    env = env.env(**config["env"]["env_kwargs"]).unwrapped
    env.reset()
    ...
    for i in range(config["training"]["episodes"]):
        ...
        trunc = [False for _ in range(len(agents))]
        ...
        for index, agent_name in enumerate(env.agents):
            ...
            obs, rew, dones[i], trunc[i], infos[i]  = env.step(actions)
            if not dones[i]:
            ...
            if remaining_timesteps < 0 or all(dones):
                # print(score[-1])
                break
